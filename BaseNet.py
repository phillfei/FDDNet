...
class BaseNet(nn.Module):
    def __init__(self, num_class, channel_list=[64, 128, 256, 512], transform_feat=128):
        ...
        # self.contrast_attn4 = ContrastiveAttention(channel_list[3])
    ...
    def forward(self, x):
        ...
        x111 = self.catconv1(torch.cat([diff1_enhanced, xA1], dim=1))
        x222 = self.catconv2(torch.cat([diff2_enhanced, xA2], dim=1))
        x333 = self.catconv3(torch.cat([diff3_enhanced, xA3], dim=1))
        x444 = self.catconv4(torch.cat([diff4_enhanced, xA4], dim=1))
